...
import type { LinePayOrder, LinePayResponse } from '../../types/index.js'
...
export const requestPayment = async (
  ...
  try {
    ...
    // 深拷貝一份order資料
    const reservation = JSON.parse(JSON.stringify(order)) as LinePayOrder & {
      returnCode: string
...
export const confirmPayment = async (transactionId: string, amount: number = 0): Promise<LinePayResponse> => {
  if (!transactionId) {
...
export const checkPaymentStatus = async (transactionId: string): Promise<LinePayResponse> => {
  try {
