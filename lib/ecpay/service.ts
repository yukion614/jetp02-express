 ...
 */
export const getECPayParams = (amount: number = 0, items: string = ''): ECPayResult => {
  let itemName: string =
  ...
  let ParamsBeforeCMV: Partial<ECPayParams> = {
   ...
   * @private
   */
  function CheckMacValueGen(parameters: Record<string, any>, algorithm: string, digest: string): string {
    let Step0: string
     ...
     * @private
     */
    function DotNETURLEncode(string: string): string {
    ...
    const Step5 = crypto.createHash(algorithm).update(Step4).digest(digest as any)
  ...
  // 四、組合最終參數
  const AllParams: ECPayParams = { ...ParamsBeforeCMV, CheckMacValue } as ECPayParams
 ...
 * @see {@link https://developers.ecpay.com.tw/?p=2856} ECPay 技術文件
 */
export const getHtmlFormContent = (action: string, params: ECPayParams): string => {
